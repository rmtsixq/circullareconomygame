...
export class City extends THREE.Group {
  ...
  constructor(size, name = 'My City') {
    ...
    this.services = [];
    // PowerService removed - using energy pool system now
   ...
   * @type {number} currentTick Current simulation tick
   */
  simulate(steps = 1, currentTick = 0) {
    let count = 0;
    while (count++ < steps) {
      // Update services
      ...
      for (let x = 0; x < this.size; x++) {
        for (let y = 0; y < this.size; y++) {
          ...
          if (tile && tile.building) {
            // Pass currentTick to building simulate
            if (typeof tile.building.simulate === 'function') {
              tile.building.simulate(this, currentTick);
            } else {
              tile.building.simulate(this);
            }
          } else {
            tile.simulate(this);
          }
    ...
    if (tile && !tile.building) {
      ...
      if (window.gameState && window.levelUnlocks) {
        ...
        const buildingUnlocks = {
          'commercial': 'eco-shop', // Level 5
          'farming': 'farming-area', // Level 7
          'textile-factory': 'textile-factory', // Level 1 (always unlocked)
      ...
      if (!building) {
        console.error(`Failed to create building of type: ${buildingType}`);
        return;
      }
      
      // Check cost for factories and power plants
