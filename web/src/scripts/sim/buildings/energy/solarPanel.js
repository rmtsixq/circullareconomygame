...
/**
 * Solar Panel Array - Produces energy from sunlight
 */
export class SolarPanel extends Building {
  type = BuildingType.solarPanel;
  
  /**
   * Current level of the solar panel (1-3)
   * @type {number}
   */
  level = 1;
  ...
  constructor(x = 0, y = 0) {
    ...
    // Solar panels don't consume energy, they produce it
  ...
  get energyProduction() {
    return this.energyProductionByLevel[this.level] || 5;
  }

  /**
   * Upgrade solar panel to next level
  ...
  refreshView() {
    ...
    if (!window.assetManager.models || !window.assetManager.models[modelName]) {
      console.error(`Model not found: ${modelName} for solar panel`);
      return;
    }
  ...
  /**
   * Simulate solar panel for one tick
   * @param {City} city 
   */
  simulate(city) {
    super.simulate(city);
  ...
  toHTML() {
    ...
    html += `
      <div class="info-heading">☀️ Güneş Paneli</div>
