...
/**
 * Wind Turbine - Produces energy from wind
 */
export class WindTurbine extends Building {
  ...
  energyProductionByLevel = {
    1: 12,  // 12 energy per tick (more than solar: 8)
    2: 18,  // 18 energy per tick (more than solar: 13)
    3: 28   // 28 energy per tick (more than solar: 20)
  };

  constructor(x = 0, y = 0) {
    super(x, y);
    this.name = 'Wind Turbine';
    // Wind turbines don't consume energy, they produce it
  ...
  get energyProduction() {
    return this.energyProductionByLevel[this.level] || 8;
  }

  /**
   * Upgrade wind turbine to next level
  ...
  /**
   * Simulate wind turbine for one tick
   * @param {City} city 
   */
  simulate(city) {
    super.simulate(city);
